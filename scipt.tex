...
\begin{definition}
A \textbf{linear region} of $F \in Rat(d, m)$ is a maximal, with respect to inclusion, connected subset of the domain on which $F$ is linear. The number of linear regions of $F$ is denoted $\mathcal{N}(f)$ \cite[p.~4]{zhang2018tropical}.
\end{definition}
