...
Then, to put this knowledge to use, we formulate statements about decision boundaries of neural networks in section \ref{sec:tropical_geometry_of_neural_networks} with tropical geometry, or more concrete, the link between neural networks and tropical rational functions. This needs fundamental 	knowledge of tropical hypersurfaces introduced in section \ref{sec:tropical_hypersurfaces}. \\

This thesis follows the underlying paper "Tropical Geometry of Deep Neural Networks" by Liwen Zhang, Gregory Naitzat and Le	k-Heng Lim very closely, in that all main statements may be reordered and rewritten, but come directly out of \cite{zhang2018tropical} and are cited accordingly.
...
The neutral element for the tropical sum is $- \infty = - \infty \oslash x = x \oslash - \infty \ x \in \mathbb{T}$. Both $0, -\infty \in \mathbb{T}$ are also a tropical function and a tropical rational function. Since for $f(x) \in \mathbb{T}(X_1, \dots , X_r)$ or $f(x) \in \mathbb{T}[X_1, \dots , X_r]$, the following hold
\begin{align*}
...
\caption{A representation of a neuron. The axon terminal attach to dendrite of another neuron. This way impulses pass from neuron to neuron\cite{neuron2020}.}
\label{fig:neuron}
\end{figure}
...
With the introduction of feedforward neural networks we have cast a sensible form of neural network with which we will be able to store and also learn complex relations between input data and output data through forward and backward propagation.\\
...
\label{fig:bool_gate}
\end{wrapfigure}
neural networks and binary outputs, like $0$ if it depicts a dogs or $1$ if it depicts a cat, are expected.
...
\end{align*}
The graph of the NOT gate is different in that it only has one input. Therefore the graph also changes to $G=(E, V)$, $V=\{ b_{11}, b_{21}, b_{31} \}$, $E=\{ (b_{11}, b_{21}), (b_{21}, b_{31})\}$ also a fully connected $2$ feedforward neural network. With weights
...
\end{align*}
Where sigma and rho bar are defined as:
\begin{align*}
...
\end{align*}
We can use the property that the derivative of the cost function is the product of derivatives between each layer and the fact, that $\overline{\sigma^{(l)}}$ and $\overline{\rho^{(l)}}$ get calculated in and my be saved in a forward propagation, which is needed to calculate $x$ before backpropagating. The affine linear functions $\rho^{(l)}$ for $l=L, \dots , 1$ can be written unambiguously as it's weight matrix $w^{(l)}$ and bias vector $b^{(l)}, \rho^{(l)} = x^{(l)} x + b^{(l)}$. With this in mind we set $x$ constant again and in the $l$-th step view $w^{(l)}$ and $b^{(l)}$ as variable again. Then we set
...
$\lceil \log_{2} r \rceil + 1$ layers. Now once more we write $p \oslash q$, where $p$ and $q$ are tropical polynomials as a neural network. Under the same assumptions as above and with
\begin{align*}
(p \oslash q)(x)
...
The linearity of a tropical polynomial per definition will be broken in its hypersurface and will be linear everywhere else. Therefore the tropical hypersurface $\mathcal{T}(f)$ of a tropical polynomial $f$ divides the domain of f into convex cells on each of which f is linear. Tropical hypersurfaces of polynomials in two variables are called \textbf{tropical curves}.\cite[p.~3]{zhang2018tropical}
...
Our analysis of neural networks requires figuring out
...
The analysis of neural

And now we come to the main corollary of this section which is important to understand section \ref{sec:tropical_geometry_of_neural_networks}.
...
\end{corollary}
\begin{proof}
Let $V_{1}$ be the vertices of the upper and $V_{2}$ the vertices of the lower face of P in relation to the projection $\pi$. As a consequence of (i), $P_{1}, \dots , P_{k}$ are nonparallel and from \ref{theo:gritzmann_sturmfels} it follows the number of vertices of $\mathcal{P}$ is 
...
\end{align*}
for all $x, y \in U_i$ and $\lambda \in [0,1] \subset \mathbb{R}$ the tropical rational function $f \oslash g$ will also be concave on $U_i$. Now because $f \oslash g$ are concave on $U_i$, which themselves are convex, we can show that the set $\{x \in U_i : f \oslash g(x) \geq c'\}$ must be convex:
\begin{align*}
(f \oslash g) (\lambda x + (1-\lambda)y)
&= \lambda f(x) + (1-\lambda) f(y) - g(\lambda x + (1-\lambda)y) \\
&\geq \lambda (f \oslash g)(x) + (1-\lambda) (f \oslash g) (y) \\
...
This thesis has followed the paper \cite{zhang2018tropical} that introduces basic knowledge for a new field of research and from which new insights to neural networks may follow.
